...
int IRrecv::getIRRemoteCode()
{
    ...
    int result = 0; //##Se determin� que devolver 0 en vez de -1 cuando no recibe nada coherente, es mejor
                    //para algunas actividades, como la m�sica con el control remoto (sobre todo cuando se
    ...
    if (decode(&results))
    {
        ...
        if (results.decode_type == RC5)  //By now, only supports RC5 Format.
        {
            ...
            if ( (strcmp(command, "0") == 0) || (strcmp(command, "800") == 0)) //##Ver si el cero es correcto.
                ...
                result = 9;
	    else if ( strcmp(command, "811") == 0) // Flecha izq
                result = 811;
	    else if ( strcmp(command, "820") == 0) //Flecha arr
                result = 820;
	    else if ( strcmp(command, "821") == 0) //Flecha Ab
                result = 821;
	    else if ( strcmp(command, "810") == 0) //Flecha Der ??????? NO
                result = 810;
